...
| **Testes** | `pytest` & `pytest-asyncio` | 260 testes unitários e de integração com banco em memória (`:memory:`) |
...
O projeto conta com **260 testes assíncronos** cobrindo repositórios, serviços, views, comparador de versões, persistência e integridade do banco:
```bash
pytest -v
```
...
1. **Integração Contínua (CI - `.github/workflows/ci.yml`)**:
   ...
   - Executa a suíte completa de 260 testes unitários e de integração no ambiente Linux com Python 3.14.
